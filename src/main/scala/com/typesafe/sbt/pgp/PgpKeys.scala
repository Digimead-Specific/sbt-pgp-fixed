...
object PgpKeys {
  ...
  // Publishing settings
  val publishSignedConfiguration = TaskKey[PublishConfiguration]("publish-signed-configuration", "Configuration for publishing to a repository.")
  val publishLocalSignedConfiguration = TaskKey[PublishConfiguration]("publish-local-signed-configuration", "Configuration for publishing to the local repository.")
  val signedArtifacts = TaskKey[Map[Artifact,File]]("signed-artifacts", "Packages all artifacts for publishing and maps the Artifact definition to the generated file.")
  val publishSigned = TaskKey[Unit]("publish-signed", "Publishing all artifacts, but SIGNED using PGP.")
  val publishLocalSigned = TaskKey[Unit]("publish-local-signed", "Publishing all artifacts to a local repository, but SIGNED using PGP.")
}